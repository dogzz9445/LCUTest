    ...
    public class LCUClient
    {
        ...
        public bool Connect()
        {
            try
            {
                ...
                httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(Encoding.ASCII.GetBytes($"riot:{LCUData.ToKen}")));
                httpClient.BaseAddress = new Uri(LCUData.ApiUrl);

                LCUData.clientProcess.EnableRaisingEvents = true;
                LCUData.clientProcess.Exited += ClientProcess_Exited;
